    ...
    if flying then
        ...
        task.spawn(function()
            while flying do
                ...
                if not hrp or not bg or not bv then break end
...
-- Генерация кнопок анимаций в разделе "Аним"
for packName, anims in pairs(AnimationPacks) do
    local PackBtn = Instance.new("TextButton")
    PackBtn.Parent = PageAnim
    PackBtn.BackgroundColor3 = Color3.fromRGB(32, 24, 48)
    PackBtn.Size = UDim2.new(1, 0, 0, 42)
    PackBtn.Font = Enum.Font.GothamBold
    PackBtn.Text = "📦 " .. packName
    PackBtn.TextColor3 = Color3.fromRGB(230, 200, 255)
    PackBtn.TextSize = 13

    local PackCorner = Instance.new("UICorner")
    PackCorner.CornerRadius = UDim.new(0, 10)
    PackCorner.Parent = PackBtn
    ...
    PackBtn.MouseButton1Click:Connect(function()
        ...
        if char then
            local animate = char:FindFirstChild("Animate")
            if animate then
                if not next(defaultAnimBackup) then
                    backupDefaultAnimations(animate)
                end
...
-- Кнопка сброса анимаций
local ResetAnimBtn = Instance.new("TextButton")
ResetAnimBtn.Parent = PageAnim
ResetAnimBtn.BackgroundColor3 = Color3.fromRGB(150, 40, 60)
ResetAnimBtn.Size = UDim2.new(1, 0, 0, 42)
ResetAnimBtn.Font = Enum.Font.GothamBold
ResetAnimBtn.Text = "🔄 Сбросить анимации"
ResetAnimBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
ResetAnimBtn.TextSize = 13

local ResetAnimCorner = Instance.new("UICorner")
ResetAnimCorner.CornerRadius = UDim.new(0, 10)
ResetAnimCorner.Parent = ResetAnimBtn

ResetAnimBtn.MouseButton1Click:Connect(function()
    restoreDefaultAnimations()
end)

-- Вкладка владельца (Info)
local OwnerLabel = Instance.new("TextLabel")
OwnerLabel.Parent = PageInfo
OwnerLabel.BackgroundTransparency = 1
OwnerLabel.Size = UDim2.new(1, 0, 0, 50)
OwnerLabel.Font = Enum.Font.GothamBold
OwnerLabel.Text = "👑 Создатель скрипта:\nKermons"
OwnerLabel.TextColor3 = Color3.fromRGB(220, 180, 255)
OwnerLabel.TextSize = 15
